...
namespace QuizIT.Service.Services
{
    public class CategoryService : ICategoryService
    {
        ...
        public CategoryService(QuizITContext dbContext)
        {
        ...
        public ServiceResult<Category> GetPage(FilterCategory filter)
        {
            ...
            try
            {
                ...
                serviceResult.Result = _dbContext.Category
                    .Where(c => c.CategoryName.ToLower().Contains(filter.Name.ToLower()))
                    .OrderByDescending(c => c.Id)
                    .Skip((filter.PageNumber - 1) * filter.PageSize)
                    .Take(filter.PageSize)
                    .ToList();
                serviceResult.TotalRecord = _dbContext.Category
        ...
        public async Task<ServiceResult<string>> Update(Category category)
        {
            ...
            try
            {
                ...
                if (serviceResult.ResponseCode != ResponseCode.BAD_REQUEST)
                {
                    ...
                    Category categoryOld = _dbContext.Category.FirstOrDefault(c => c.Id == category.Id);
                    //Sai id
                    if (categoryOld == null)
                    {
                    ...
                    _dbContext.Category.Update(categoryOld);
                    await _dbContext.SaveChangesAsync();
                }
        ...
        public async Task<ServiceResult<string>> Delete(Category category)
        {
            ...
            try
            {
                ...
                if (_dbContext.Exam.FirstOrDefault(e => e.CategoryId == category.Id) != null ||
                    _dbContext.Question.FirstOrDefault(q => q.CategoryId == category.Id) != null)
                {
                ...
                {
                    _dbContext.Category.Remove(_dbContext.Category.FirstOrDefault(c => c.Id == category.Id));
                    await _dbContext.SaveChangesAsync();
                }
        ...
        private void validateCategory(Category category, ref ServiceResult<string> serviceResult, bool isUpdate = false)
        {
            ...
            if (isUpdate == false)
            {
                if (_dbContext.Category.FirstOrDefault(c => c.CategoryName.ToLower() == category.CategoryName.ToLower()) != null)
                {
                    serviceResult.ResponseCode = ResponseCode.BAD_REQUEST;
            ...
            {
                if (_dbContext.Category.FirstOrDefault(c => c.CategoryName.ToLower() == category.CategoryName.ToLower() && c.Id != category.Id) != null)
                {
                    serviceResult.ResponseCode = ResponseCode.BAD_REQUEST;
                    serviceResult.ResponseMess = EXISTS_CATEGORY_NAME;
                }
