...
namespace QuizIT.Web.Controllers
{
    public class HistoryController : BaseController
    {
        ...
        private List<ExportHistory> GetLstExportHistory(History history)
        {
            ...
            var exam = JsonConvert.DeserializeObject<Exam>(history.JsonExamp);
            ...
            foreach (var historyDetail in history.HistoryDetail)
            {
                var question = exam
                    .ExamDetail
                    .Where(x => x.QuestionId == historyDetail.QuestionId)
                    .FirstOrDefault().Question;

                ExportHistory exportHistory = new ExportHistory
                {
                    QuestionContent = $"Câu {order}. {question.Content}"
                };
                switch (historyDetail.AnswerSelect)
                {
                    case "A":
                        exportHistory.AnswerSelect = $"A. {question.AnswerA}";
                        exportHistory.Result = (question.AnswerCorrect == "A" ? "Đúng" : "Sai");
                        break;

                    case "B":
                        exportHistory.AnswerSelect = $"B. {question.AnswerB}";
                        exportHistory.Result = (question.AnswerCorrect == "B" ? "Đúng" : "Sai");
                        break;

                    case "C":
                        exportHistory.AnswerSelect = $"C. {question.AnswerC}";
                        exportHistory.Result = (question.AnswerCorrect == "C" ? "Đúng" : "Sai");
                        break;

                    case "D":
                        exportHistory.AnswerSelect = $"D. {question.AnswerD}";
                        exportHistory.Result = (question.AnswerCorrect == "D" ? "Đúng" : "Sai");
                        break;
